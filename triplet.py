...
class EntityDict:
    ...
    def __init__(self, entity_dict_dir: str, inductive_test_path: str = None):
        ...
        type_path = os.path.join(entity_dict_dir, 'entity2type.json')
        assert os.path.exists(path) and os.path.exists(type_path)
        self.entity_exs = [EntityExample(**obj) for obj in json.load(open(path, 'r', encoding='utf-8'))]
        self.entity_types = json.load(open(type_path, 'r', encoding='utf-8'))
        for i in range(len(self.entity_exs)):
            self.entity_exs[i].entity_type = self.entity_types[self.entity_exs[i].entity_id]
