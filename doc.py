...
class Example:
    ...
    def head_type(self):
        if self.head_id == '':
            return ''
        return ', '.join(entity_dict.get_entity_by_id(self.head_id).entity_type)
    ...
    def vectorize(self) -> dict:
        ...
        schema_encoded_inputs = _custom_tokenize(text=self.head_type, text_pair=self.relation)

        tail_word = _parse_entity_name(self.tail)
        tail_text = _concat_name_desc(tail_word, tail_desc)
        tail_encoded_inputs = _custom_tokenize(text=tail_text)

        return {'head_token_ids': head_encoded_inputs['input_ids'],
                'head_token_type_ids': head_encoded_inputs['token_type_ids'],
                'schema_token_ids': schema_encoded_inputs['input_ids'],
                'schema_token_type_ids': schema_encoded_inputs['token_type_ids'],
...
def collate(batch_data: List[dict]) -> dict:
    ...
    schema_token_ids, schema_mask = to_indices_and_mask(
        [torch.LongTensor(ex['schema_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    schema_token_type_ids = to_indices_and_mask(
        [torch.LongTensor(ex['schema_token_type_ids']) for ex in batch_data],
        need_mask=False)
    ...
    batch_dict = {
        ...
        'schema_token_type_ids': schema_token_type_ids,
